...
function ProjectionsTab({ data }) {
  ...
  const { projectionsData, growthPotential3Years, formatRupiah } = data;
  ...
  const fullProjection = growthPotential3Years?.monthlyProjections || [];

  const year1Profit = fullProjection[11]?.cumulativeProfit || 0;
  const year3Profit = fullProjection[35]?.cumulativeProfit || year1Profit;
